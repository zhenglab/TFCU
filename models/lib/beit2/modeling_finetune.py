...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        ...
        for blk in self.blocks:
            x, attn = blk(x, rel_pos_bias=rel_pos_bias)
            x_list.append(x)
        x = self.norm(x)
    ...
    def forward_features_first6layer(self, x, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        if len(x.size())==4:
            B, nc, w, h = x.shape
        elif len(x.size())==5:
            B, c, t, w, h = x.shape
    ...
    def forward_features_last6layer(self, x, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        rel_pos_bias = self.rel_pos_bias() if self.rel_pos_bias is not None else None
        x_list = []
        x_list.append(x)
        for i,blk in enumerate(self.blocks):
