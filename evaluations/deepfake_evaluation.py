...
def predict_video_from_frames(frames_pred):
    predict_score = []
    if frames_pred == '[-1]':
        return 0
    for i, v in enumerate(eval(frames_pred)):
        predict_score.append(v[1])
...
if __name__ == '__main__':
    ...
    frames_predict = results['predict'].values
    video = results['video'].values

    preds = []
    with tqdm(total=len(video)) as pbar:
        for i, name in enumerate(video):
            preds.append(predict_video_from_frames(frames_predict[i]))
            pbar.update()
    
    video_thres, video_acc, video_f1=acc_f1_eval(label,preds)
    video_auc=AUC(label,preds)
    video_log_loss = log_loss(label, preds, labels=[0, 1])
    rs={'thr':video_thres,'acc':video_acc,'auc':video_auc, 'log':video_log_loss,'f-1':video_f1}
    ...
    print(dt)
    print(opt.result_root.split("/")[-1][:-4])
    for k, v in rs.items():
        print("%s: %.4f" % (k, v))
    # print(rs)
