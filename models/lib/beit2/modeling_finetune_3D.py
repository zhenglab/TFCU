...
class Attention(nn.Module):
    def __init__(
        ...
        window_num = num_frames//tubelet_size
        ...
        if window_size:
            ...
            self.num_relative_distance = self.window_num *(2 * window_size[0] - 1) * (2 * window_size[1] - 1) + 3
            ...
            coords_n = torch.arange(self.window_num)
            coords = torch.stack(torch.meshgrid([coords_h, coords_w, coords_n]))  # 2, Wh, Ww
            ...
            relative_position_index = \
                torch.zeros(size=(self.window_num * window_size[0] * window_size[1] + 1, ) * 2, dtype=relative_coords.dtype)
        ...
        if self.relative_position_bias_table is not None:
            ...
            relative_position_bias = \
                self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
                    self.window_num *self.window_size[0] * self.window_size[1] + 1,
                    self.window_num *self.window_size[0] * self.window_size[1] + 1, -1)  # Wh*Ww,Wh*Ww,nH
            # print(self.relative_position_index.shape, relative_position_bias.shape)
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                init_values=init_values, window_size=self.patch_embed.patch_shape if use_rel_pos_bias else None, num_frames=num_frames, tubelet_size=tubelet_size)
