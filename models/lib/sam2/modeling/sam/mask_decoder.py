    ...
    def predict_masks(
        ...
        if self.pred_obj_scores:
            output_tokens = torch.cat(
                ...
                dim=0,
            ) # torch.Size([1, 768]) torch.Size([1, 768]) torch.Size([4, 768])
            s = 1
        else:
            output_tokens = torch.cat(
        ...
        hs, src, attn_TokentoImage, attn_ImagetoToken = self.transformer(src, pos_src, tokens)
        ...
        return masks, iou_pred, mask_tokens_out, object_score_logits, img_token_attn, attn_TokentoImage, attn_ImagetoToken
